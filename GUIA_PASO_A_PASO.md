...
- `k8s/infrastructure/argocd/applications/observability-prometheus-app.yaml`
- `k8s/infrastructure/argocd/applications/observability-loki-app.yaml`
- `k8s/infrastructure/argocd/applications/observability-promtail-app.yaml`
- `k8s/infrastructure/argocd/applications/observability-tempo-app.yaml`
- `k8s/infrastructure/argocd/applications/backup-app.yaml`
...
### 1.5.1 Crear la variable REGISTRY_OWNER en GitHub

El pipeline usa una variable de GitHub para identificar el owner del registry de imágenes.
Esto evita depender del usuario que hace push y hace el pipeline portable.

Ve a tu repo en GitHub > **Settings** > **Secrets and variables** > **Actions** > **Variables** > **New repository variable**:

| Campo | Valor |
|-------|-------|
| Name  | `REGISTRY_OWNER` |
| Value | tu usuario de GitHub en minúsculas (ej: `rubenlopsol`) |

> Esta variable se usa en `ci.yml` y `cd.yml` para construir las URLs de las imágenes:
> `ghcr.io/${{ vars.REGISTRY_OWNER }}/openpanel-api:main-<sha>`
...
```bash
# PostgreSQL
kubectl apply -f k8s/apps/base/openpanel/postgres-statefulset.yaml
kubectl apply -f k8s/apps/base/openpanel/postgres-service.yaml
...
```bash
# ClickHouse
kubectl apply -f k8s/apps/base/openpanel/clickhouse-statefulset.yaml
kubectl apply -f k8s/apps/base/openpanel/clickhouse-service.yaml
...
```bash
# Redis
kubectl apply -f k8s/apps/base/openpanel/redis-deployment.yaml
kubectl apply -f k8s/apps/base/openpanel/redis-service.yaml
...
```bash
kubectl apply -f k8s/apps/base/openpanel/configmap.yaml
```
...
kubectl apply -f k8s/apps/base/openpanel/api-deployment-blue.yaml
kubectl apply -f k8s/apps/base/openpanel/api-service.yaml

# Dashboard
kubectl apply -f k8s/apps/base/openpanel/start-deployment.yaml
kubectl apply -f k8s/apps/base/openpanel/start-service.yaml

# Worker
kubectl apply -f k8s/apps/base/openpanel/worker-deployment.yaml
```
...
```bash
kubectl apply -f k8s/apps/base/openpanel/ingress.yaml
```
...
```bash
kubectl apply -f k8s/apps/base/openpanel/network-policies.yaml
...
- `kustomize build` falla → Algun archivo listado en un `kustomization.yaml` no existe. Revisa `k8s/apps/overlays/staging/kustomization.yaml` y `k8s/apps/base/*/kustomization.yaml`
...
ArgoCD se instala via **Helm** con el script incluido:

```bash
./scripts/install-argocd.sh
```

El script añade el repo Helm de ArgoCD, instala el chart `argo/argo-cd` en modo HTTP (sin TLS) y crea el Ingress para `argocd.local`. Al finalizar muestra la contraseña inicial del admin.

### 7.2 Parchear argocd-server para modo HTTP (sin TLS)

> Con la instalación via Helm ya no es necesario este paso — el script pasa `--set server.insecure=true` directamente al chart. Esta sección se mantiene solo como referencia si se instaló con kubectl apply.

```bash
# Solo necesario si NO se usó el script install-argocd.sh
...
```bash
kubectl apply -f k8s/infrastructure/argocd/projects/
kubectl apply -f k8s/infrastructure/argocd/applications/
```
...
# Edita k8s/apps/base/openpanel/start-deployment.yaml y cambia replicas: 1 a replicas: 2
git add k8s/apps/base/openpanel/start-deployment.yaml
...
kubectl create secret generic postgres-credentials \
  ...
  kubeseal --format=yaml > k8s/infrastructure/base/sealed-secrets/secrets.yaml
...
```bash
kubectl apply -f k8s/infrastructure/base/sealed-secrets/
```
...
Los archivos en `k8s/infrastructure/base/sealed-secrets/` contienen los secretos cifrados con la clave
del controlador. Son seguros para subir a Git publico o privado.

```bash
git add k8s/infrastructure/base/sealed-secrets/
...
El stack de observabilidad se despliega via **Helm charts oficiales** gestionados por ArgoCD.
No hay YAMLs manuales — todo se configura en `k8s/infrastructure/observability/`.

### 9.1 Verificar que ArgoCD desplegó las 4 apps de observabilidad

```bash
kubectl get applications -n argocd | grep observability
# Deben aparecer 4 apps: observability-prometheus, observability-loki, observability-promtail, observability-tempo
# Estado: Synced + Healthy

kubectl get pods -n observability
# Deben estar corriendo pods de prometheus, grafana, loki, promtail (daemonset), tempo
```

Si no estan desplegados, aplica manualmente:

```bash
kubectl apply -f k8s/infrastructure/argocd/applications/observability-prometheus-app.yaml
kubectl apply -f k8s/infrastructure/argocd/applications/observability-loki-app.yaml
kubectl apply -f k8s/infrastructure/argocd/applications/observability-promtail-app.yaml
kubectl apply -f k8s/infrastructure/argocd/applications/observability-tempo-app.yaml
```
...
kubectl port-forward svc/observability-prometheus-prometheus 9090:9090 -n observability &
...
# Login: admin / admin (configurable en k8s/infrastructure/observability/kube-prometheus-stack.yaml)

# O via port-forward
kubectl port-forward svc/observability-prometheus-grafana 3000:3000 -n observability
```

### 9.4 Verificar datasources en Grafana

Los datasources (Prometheus, Loki, Tempo) se configuran automáticamente via el values file.
...
- Deben aparecer las alertas definidas en `k8s/infrastructure/observability/kube-prometheus-stack.yaml`
...
> **NOTA — Versión de Tempo:**
> El chart `grafana/tempo` v1.10.3 ya usa una versión estable de Tempo. La versión se controla en `k8s/infrastructure/observability/tempo.yaml`.
> Si hay problemas de compatibilidad, ajusta `targetRevision` en `observability-tempo-app.yaml` a una versión anterior.
...
> ```bash
> git add k8s/apps/base/openpanel/configmap.yaml  # o el deployment que hayas modificado
...
El deployment green (`k8s/apps/base/openpanel/api-deployment-green.yaml`) debe tener los mismos fixes que blue:
...
```bash
kubectl apply -f k8s/apps/base/openpanel/api-deployment-green.yaml
```
...
El repositorio incluye `credentials-velero.example` como plantilla. Copialo y rellena tus valores:
...
# Verificar que esta en .gitignore (nunca debe subirse al repo)
grep "credentials-velero" .gitignore
# Debe devolver: credentials-velero
```

> IMPORTANTE: Nunca hagas `git add credentials-velero`. Solo `credentials-velero.example` va en el repositorio.
